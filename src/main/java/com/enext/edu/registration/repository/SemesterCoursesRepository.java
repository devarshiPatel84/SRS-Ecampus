...
public interface SemesterCoursesRepository extends JpaRepository<SemesterCourses, Integer> {
    ...
    // lPassSql = "SELECT SRCSCRID FROM STUDENTREGISTRATIONS,STUDENTREGISTRATIONCOURSES,STUDENTSCORES " +
	// 			   " WHERE SRCROWSTATE > 0 AND SRGROWSTATE > 0 AND SSCROWSTATE > 0 " + 
	// 			   " AND SRGID = SRCSRGID AND SRCTCRID = SSCTCRID AND SSCSTDID = SRGSTDID " +
	// 			   " AND SRGSTDID = " + lStdId;

    // lSql = "SELECT * FROM SEMESTERCOURSES,COURSEGROUPS,SEMESTERS WHERE SCRROWSTATE>0 AND STRROWSTATE > 0 " +
	// 		   " AND CGPROWSTATE>0 AND CGPID=SCRCGPID AND STRID=SCRSTRID AND SCRELECTIVE='" +
	// 		   CourseConstants.CODEVALUESEMESTERCOURSEELECTIVEY + "' AND STRBCHID = " + 
	// 			lSemesterBean.getBatchBean().getId().toString() + " AND STRID < " + lStrId +
	// 		   " AND SCRID NOT IN (" + lPassSql + ")";
