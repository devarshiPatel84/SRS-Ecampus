...
public interface TermCoursesRepository extends JpaRepository<TermCourses, Integer> {
    ...
    // lSql = "SELECT * FROM TERMCOURSEAVAILABLEFOR,TERMCOURSES,COURSES WHERE TCAPRGID="+prgId+" AND TCRTRMID="+trmId+" AND TCATCRID=TCRID  AND TCRCRSID=CRSID AND TCRCRSID IN (SELECT TCRCRSID FROM EGCRSTT1,TERMCOURSES WHERE TERMCOURSES.TCRID=EGCRSTT1.TCRID AND (OBTGR_ID=4 OR  OBTGR_ID=21 OR OBTGR_ID=22) AND  ROW_ST>0 AND STUD_ID="+lStdId+" AND TCRROWSTATE>0  AND TCRCRSID=CRSID AND CRSROWSTATE>0) AND TCRROWSTATE>0 AND TCRROWSTATE>0 AND CRSROWSTATE>0 ORDER BY CRSNAME";
